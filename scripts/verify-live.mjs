...
assert(axe.violations.length === 0, `root has axe violations: ${axe.violations.map((item) => item.id).join(', ')}`);
...
axe = await new AxeBuilder({ page }).analyze();
assert(axe.violations.length === 0, `demo has axe violations: ${axe.violations.map((item) => item.id).join(', ')}`);
...
for (const [route, title] of [['/demo', 'Demo — Patchboard'], ['/privacy', 'Privacy — Patchboard'], ['/terms', 'Terms — Patchboard']]) {
  ...
  axe = await new AxeBuilder({ page }).analyze();
  assert(axe.violations.length === 0, `${route} has axe violations: ${axe.violations.map((item) => item.id).join(', ')}`);
}
...
axe = await new AxeBuilder({ page }).analyze();
assert(axe.violations.length === 0, `404 has axe violations: ${axe.violations.map((item) => item.id).join(', ')}`);
...
for (const route of ['/', '/?demo=1', '/privacy', '/terms', '/404.html']) {
  ...
  const undersized = await mobile.evaluate(() => [...document.querySelectorAll('a, button, input:not([type="checkbox"]), select, textarea, label:has(input[type="checkbox"])')].flatMap((element) => {
    const style = getComputedStyle(element);
    const box = element.getBoundingClientRect();
    if (style.visibility === 'hidden' || style.display === 'none' || box.width === 0 || box.height === 0) return [];
    return box.width >= 44 && box.height >= 44 ? [] : [element.getAttribute('aria-label') || element.textContent?.trim() || element.id];
  }));
  assert(undersized.length === 0, `${route} has undersized touch targets: ${undersized.join(', ')}`);
  await mobile.evaluate(() => { document.documentElement.style.fontSize = '200%'; });
  const resized = await mobile.evaluate(() => {
    const viewportWidth = document.documentElement.clientWidth;
    const clipped = [...document.querySelectorAll('a, button, input, select, textarea')].flatMap((element) => {
      const style = getComputedStyle(element);
      const box = element.getBoundingClientRect();
      if (style.visibility === 'hidden' || style.display === 'none' || box.width === 0 || box.height === 0) return [];
      return box.left >= -0.5 && box.right <= viewportWidth + 0.5 ? [] : [element.getAttribute('aria-label') || element.textContent?.trim() || element.id];
    });
    return { scrollWidth: document.documentElement.scrollWidth, viewportWidth, clipped };
  });
  assert(resized.scrollWidth <= resized.viewportWidth, `${route} overflows at 200% text`);
  assert(resized.clipped.length === 0, `${route} clips controls at 200% text: ${resized.clipped.join(', ')}`);
}
...
await browser.close();
console.log(JSON.stringify({ base, routes: 5, unknownStatus: 404, axeViolations: 0, consoleErrors: 0, externalRequests: 0, mobileOverflow: false, textResizeOverflow: false, undersizedTouchTargets: 0, offlineReload: true }));
